...
///Método añadido por Ignacio, para leer ordenadamente direcciones de la memoria no consecutivas
//Aplicable para la lectura de nombres largos almacenados en entradas de la tabla root

MMCSD_err mmcsd_read_long_name(uint32_t ComienzoMemoria, uint16_t Nloops, uint8_t* ptr)
{
	...
	for(int ContLoopsNL=1; ContLoopsNL<Nloops+1; ContLoopsNL++ )
	{
		//Nos posicionamos en el bloque correspondiente
		DIR_MEMORIA_NOMBRE_RECURSIVA = ComienzoMemoria - (RESTAR_CONSTANTE * ContLoopsNL);		

		for(int ContNL=0; ContNL<13; ContNL++ )//4 //Vamos leyendo los caracteres dentro de cada bloque de 32
		{
			if(ContNL==0)
			...
			r1 = mmcsd_read_byte(DIR_MEMORIA_NOMBRE_RECURSIVA, ptr);
		    if(r1 != MMCSD_GOODEC)
		         return r1;	
...
MMCSD_err mmcsd_move_buffer(uint32_t new_addr)
{
   ...
   //cur_block = g_mmcsdBufferAddress - (g_mmcsdBufferAddress % MMCSD_MAX_BLOCK_SIZE);
	block_aux = new_addr % MMCSD_MAX_BLOCK_SIZE;
   //You will set the pointer at the beggining of the new sector
